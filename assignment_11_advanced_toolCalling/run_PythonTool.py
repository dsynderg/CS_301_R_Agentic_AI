    ...
    async def get_response(self, user_message: str):
        ...
        async with stream as stream:
            async for event in stream:
                ...
                # Detect code_interpreter tool usage
                if event.type == "response.output_item.added":
                    if hasattr(event, 'item') and hasattr(event.item, 'type'):
                        if event.item.type == 'tool_use' and hasattr(event.item, 'name'):
                            if event.item.name == 'code_interpreter':
                                tool_input = getattr(event.item, 'input', {})
                                code = tool_input.get('code', tool_input)
                                print("\n========== CODE INTERPRETER USED ==========", flush=True)
                                print(f"Code:\n{code}", flush=True)
                                print("============================================\n", flush=True)
                                yield 'reasoning', (
                                    "\n\n________CODE INTERPRETER______\n\n"
